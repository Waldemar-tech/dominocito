...
// ─── FASE 1: modos de juego ──────────────────────────────────
type GameMode = 'individual' | 'teams';
type TeamMode = 'manual' | 'choose' | 'random';

function isValidGameMode(m: unknown): m is GameMode {
  return m === 'individual' || m === 'teams';
}
function isValidTeamMode(m: unknown): m is TeamMode {
  return m === 'manual' || m === 'choose' || m === 'random';
}
...
router.post('/rooms', requireAuth, async (req: Request, res: Response) => {
  ...
  // FASE 1: validación de modo de juego.
  if (!isValidGameMode(gameMode)) {
    return res.status(400).json({ error: "gameMode debe ser 'individual' o 'teams'" });
  }
  // El modo parejas exige mesa de 4 y un team_mode válido.
  let finalTeamMode: TeamMode | null = null;
  if (gameMode === 'teams') {
    if (maxPlayers !== 4) {
      return res.status(400).json({ error: 'El modo parejas requiere 4 jugadores' });
    }
    if (!isValidTeamMode(teamMode)) {
  ...
  try {
    ...
    // En modo parejas, el host arranca en el equipo 0 (asiento 0).
    // En 'choose' podrá cambiarse luego; en 'random' se re-sortea al iniciar.
    const hostTeam = gameMode === 'teams' ? 0 : null;
    await client.query(
      `INSERT INTO dc_domino_players (room_id, user_id, position, team, is_connected)
       VALUES ($1, $2, 0, $3, true)`,
      [room.id, userId, hostTeam]
    );
...
router.post('/rooms/:code/join', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const roomResult = await client.query(
      `SELECT id, status, max_players, game_mode, team_mode FROM dc_domino_rooms WHERE code = $1 FOR UPDATE`,
      [code]
    );
    ...
    // FASE 1: asignación de equipo al entrar.
    //  - individual        → team NULL
    //  - teams / manual     → NULL (el host lo asigna con domino:set_teams)
    //  - teams / choose      → NULL (el jugador elige con domino:choose_team)
    //  - teams / random      → NULL (se sortea al iniciar)
    // O sea: al entrar, en teams el equipo queda NULL salvo el host (asiento 0).
    // El auto-relleno "al equipo con cupo" lo hace el propio jugador vía
    // choose, o el host vía set_teams. Así evitamos asignar un equipo que el
    // host no quería en modo manual.
    let joinTeam: number | null = null;

    await client.query(
      `INSERT INTO dc_domino_players (room_id, user_id, position, team, is_connected)
       VALUES ($1, $2, $3, $4, true)`,
      [room.id, userId, nextPos, joinTeam]
    );
