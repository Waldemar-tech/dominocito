        ...
        if (FichaOrigen === false) {
            ...
            switch (FichaOrigen.Direccion) {
                case "centro" :
                    ...
                    // === MODO SOCKET ===
                    // Al azar NO sirve: la segunda ficha de la partida saldria a
                    // un lado distinto en cada cliente, y el "izquierda/derecha"
                    // local quedaria espejado respecto al del servidor. A partir
                    // de ahi el servidor rechaza jugadas legales.
                    // window.__DEVIL33_FORCE_RAMA__ ('left'|'right') fuerza la rama.
                    if (FichaOrigen === Domino.Partida.FichaIzquierda && FichaOrigen === Domino.Partida.FichaDerecha) {
                        var _forzar = window.__DEVIL33_FORCE_RAMA__;
                        var _irIzq  = (_forzar === "left")  ? true
                                    : (_forzar === "right") ? false
                                    : (RandInt() === 0);
                        if (_irIzq)  { 
...
//                Domino.Partida.FichaDerecha = this;
            ...
            if (this.Direccion === "nada") {
                console.warn('[Domino_Ficha] Colocar: Direccion quedó en nada, FichaOrigen.Direccion=' + FichaOrigen.Direccion + ' Rama=' + FichaOrigen.Rama);
                // Fallback: asumir derecha si no se pudo determinar dirección
                Ret = this.BuscarPosDer(FichaOrigen);
                this.Direccion = "derecha";
                this.Rama = "derecha";
            }
        ...
        this.AniColocar = Animaciones.CrearAnimacion([
            ...
            FuncionTerminado : function() {
                ...
                // === MODO SOCKET ===
                // El turno lo manda el servidor via SyncState. Si avanzamos el
                // turno aca, cada jugada de un rival que reproducimos localmente
                // le roba el turno al jugador local (JugadorActual pasa de 0 a 1)
                // y JugadorColocar() deja de responder a sus clicks.
                if (window.__DEVIL33_SOCKET_MODE__) return;
