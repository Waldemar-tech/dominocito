...
/*        for (i = 0; i < 4; i ++) {
    ...
    this.Turno = function() {
        ...
        // SOCKET MODE: el servidor determina quién empieza y coloca la primera ficha via SyncState
        if (this.TurnoActual === 0 && !window.__DEVIL33_SOCKET_MODE__) {
...
//                    this.MostrarMensaje(this.JugadorActual, "Jugador" + (this.JugadorActual + 1) + " tira : " + this.Ficha[(this.JugadorActual * 7) + i].Valores[1] + " | " + this.Ficha[(this.JugadorActual * 7) + i].Valores[0]);
        ...
        // El resto de turnos 
            ...
            if (Posibilidades.length > 0)  {
                ...
                // Turno de la maquina
                // SOCKET MODE: no hay IA local, esperamos domino:state del servidor
                if (this.JugadorActual !== 0 && !window.__DEVIL33_SOCKET_MODE__) {
                    // IA 1.0
    ...
    this.ComprobarManoTerminada = function() {
        ...
        // SOCKET MODE: el servidor es la autoridad de fin de mano
        if (window.__DEVIL33_SOCKET_MODE__) return false;
...
//        var Rama = "izquierda";
        if (this.JugadorActual === 0) {
            ...
            // Contar fichas colocadas para detectar primera jugada
            var _colocadas = 0;
            for (var ci = 0; ci < this.Ficha.length; ci++) { if (this.Ficha[ci].Colocada) _colocadas++; }
            var _esPrimeraJugada = (_colocadas === 0);

            for (var i = 0; i < 7; i++) {
                // Es muy importante saber si la ficha está hover o no
                if (this.Ficha[i].Hover > 0 && this.Ficha[i].Colocada === false) {

                    // SOCKET MODE: primera jugada — solo vale la ficha de salida
                    // (el doble mas alto). El servidor tambien lo valida; esto
                    // evita el toast de error y el clic muerto.
                    if (_esPrimeraJugada && window.__DEVIL33_SOCKET_MODE__) {
                        var _op = window.__DEVIL33_OPENING_TILE__;
                        if (_op) {
                            var _v = this.Ficha[i].Valores;
                            var _mismo = Math.min(_v[0], _v[1]) === Math.min(_op[0], _op[1]) &&
                                         Math.max(_v[0], _v[1]) === Math.max(_op[0], _op[1]);
                            if (!_mismo) return; // no es la ficha de salida: ignorar el clic
                        }
                        this.Ficha[i].Colocar(false, true);
                        window.parent.postMessage({
                            type: 'devil33:play',
                            tile: [this.Ficha[i].Valores[0], this.Ficha[i].Valores[1]],
                            side: 'right'
                        }, '*');
                        this.OcultarAyuda();
                        return;
                    ...
                    var nPos = -1;
                    if (typeof this.FichaIzquierda.ValorLibre === 'function' &&
                        typeof this.FichaDerecha.ValorLibre === 'function' &&
                        (this.Ficha[i].Valores[0] === this.FichaIzquierda.ValorLibre() || this.Ficha[i].Valores[1] === this.FichaIzquierda.ValorLibre()) && 
                        (this.Ficha[i].Valores[0] === this.FichaDerecha.ValorLibre()   || this.Ficha[i].Valores[1] === this.FichaDerecha.ValorLibre()) && 
                        ...
                        if (typeof this.FichaDerecha.ValorLibre === 'function' &&
                            (this.Ficha[i].Valores[0] === this.FichaDerecha.ValorLibre() || this.Ficha[i].Valores[1] === this.FichaDerecha.ValorLibre())) {
                    ...
                    if (nPos !== -1) {
                        ...
                        // === MODO SOCKET ===
                        // Calculamos el lado ANTES de colocar y forzamos esa rama,
                        // para que el render local coincida con lo que reporta el
                        // servidor. Justo despues del doble de salida, FichaIzquierda
                        // y FichaDerecha son el MISMO objeto, asi que este ternario
                        // da 'left' de forma consistente.
                        var _side = (nPos === this.FichaIzquierda) ? 'left' : 'right';
                        if (window.__DEVIL33_SOCKET_MODE__) window.__DEVIL33_FORCE_RAMA__ = _side;

                        this.Ficha[i].Colocar(nPos, true);

                        if (window.__DEVIL33_SOCKET_MODE__) window.__DEVIL33_FORCE_RAMA__ = undefined;
                        this.MostrarMensaje(this.JugadorActual, "<span>" + this.Opciones.NombreJugador[this.JugadorActual] + "</span> " + 
                                            " <span " +
                                                "data-idioma-en=' throws : '" + 
                                                "data-idioma-cat=' tira : '" + 
                        ...
                        // SOCKET MODE: notificar al wrapper React en vez de avanzar turno local
                        if (window.__DEVIL33_SOCKET_MODE__) {
                            ...
                            this.OcultarAyuda();
                            return; // No avanzar turno — esperamos domino:state
                        }
