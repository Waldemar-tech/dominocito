...
(function() {
  ...
  Domino.SyncState = function(state, viewerUserId) {
    ...
    // FASE 2 (opción B-ligera): ordenar el board por 'order' (orden de juego).
    // La cadena visual de Devil33 se arma ficha-tras-ficha; si el board no viene
    // en el orden en que se jugó, FichaOrigen (la ficha anterior de la cadena)
    // puede no existir aún y BuscarPosDer/BuscarPosIzq explotan. Ordenar por
    // 'order' garantiza que cada ficha se coloque después de su predecesora.
    // NO tocamos el posicionamiento de Devil33 — solo el ORDEN de colocado.
    if (state && state.board && state.board.length > 1) {
      state = Object.assign({}, state);
      state.board = state.board.slice().sort(function(a, b) {
    ...
    // Para cada posición visual de oponente, juntar las fichas que ESE jugador
    // ya jugó (están en el board con su valor real). Esas fichas DEBEN ocupar
    // los primeros slots del oponente para que findFichaIndex las encuentre por
    // valor. El resto de slots (fichas boca abajo aún en mano del oponente) se
    // rellenan con el pool de desconocidas — su valor real es irrelevante porque
    // el jugador nunca las ve.
    var boardTilesByVisualPos = [[], [], [], []];
    for (var bi = 0; bi < state.board.length; bi++) {
      var bEntry = state.board[bi];
      var bServerPos = 0;
      for (var bk = 0; bk < state.players.length; bk++) {
        if (String(state.players[bk].userId) === String(bEntry.userId)) {
          bServerPos = state.players[bk].position; break;
        }
      }
      var bVisual = serverPosToVisualPos[bServerPos];
      if (bVisual === undefined) bVisual = bServerPos;
      if (bVisual >= 1 && bVisual <= 3) boardTilesByVisualPos[bVisual].push(bEntry.tile);
    }
    ...
    for (var vp = 0; vp < 4; vp++) {
      ...
      if (vp === 0) {
        // Mano del viewer (visual pos 0): SIEMPRE sus valores reales del server.
        // Nunca depende del pool, así que nunca queda en [0,0] por agotamiento.
        for (var j = 0; j < 7; j++) {
          var f = partida.Ficha[j];
          if (!f) continue;
          var tile = vhand[j];
          setFichaValores(f, tile ? tile : [0, 0]);
        }
      } else {
        // Oponente: primero sus fichas ya jugadas (valor real del board),
        // después el pool para las que siguen boca abajo.
        var played = boardTilesByVisualPos[vp];
        for (var j2 = 0; j2 < 7; j2++) {
          var f2 = partida.Ficha[vp * 7 + j2];
          if (!f2) continue;
          if (j2 < played.length) {
            setFichaValores(f2, played[j2]);           // ficha real que jugó
          } else if (poolIdx < unknownPool.length) {
            setFichaValores(f2, unknownPool[poolIdx]); // boca abajo: valor irrelevante
            poolIdx++;
          } else {
            setFichaValores(f2, [0, 0]);               // fallback (no visible)
          }
        }
      }
    }
    ...
    for (var b = 0; b < state.board.length; b++) {
      ...
      try {
        ...
        // FASE 2 (B-ligera): actualizar el extremo de la cadena con la ficha
        // recién colocada, para que la SIGUIENTE se enganche a ella y no a un
        // extremo viejo/undefined. El engine tiene esta actualización comentada,
        // por eso FichaOrigen llegaba undefined. Usamos la Rama que el propio
        // Colocar() asignó (respeta la lógica de posicionamiento de Devil33).
        if (b === 0) {
          // Ficha de salida (centro): es ambos extremos a la vez.
          partida.FichaIzquierda = ficha;
          partida.FichaDerecha = ficha;
        } else if (ficha.Rama === 'izquierda') {
          partida.FichaIzquierda = ficha;
        } else if (ficha.Rama === 'derecha') {
          partida.FichaDerecha = ficha;
        } else {
          // Fallback: usar el lado que reportó el server.
          if (entry.side === 'left') partida.FichaIzquierda = ficha;
          else partida.FichaDerecha = ficha;
        }
      } catch (err) {
    ...
    if (failed) {
      ...
      // Re-asignar valores usando el mapeo VISUAL correcto (mismo que el bloque
      // principal). El viewer SIEMPRE va al slot visual 0, sin importar su
      // posición en el server. Los oponentes reciben primero sus fichas jugadas
      // (valor real del board) y luego el pool para las boca abajo.
      var poolIdx2 = 0;
      for (var vp2 = 0; vp2 < 4; vp2++) {
        var vhand2 = visualHands[vp2] || [];
        if (vp2 === 0) {
          for (var j2 = 0; j2 < 7; j2++) {
            var f2 = partida.Ficha[j2];
            if (!f2) continue;
            var tile2 = vhand2[j2];
            setFichaValores(f2, tile2 ? tile2 : [0, 0]);
          }
        } else {
          var played2 = boardTilesByVisualPos[vp2];
          for (var j2b = 0; j2b < 7; j2b++) {
            var f2b = partida.Ficha[vp2 * 7 + j2b];
            if (!f2b) continue;
            if (j2b < played2.length) {
              setFichaValores(f2b, played2[j2b]);
            } else if (poolIdx2 < unknownPool.length) {
              setFichaValores(f2b, unknownPool[poolIdx2]);
              poolIdx2++;
            } else {
              setFichaValores(f2b, [0, 0]);
            }
          }
        }
      }
      ...
      for (var bb = 0; bb < state.board.length; bb++) {
        ...
        try {
          ficha2.Colocar(useRama2, false);
          // FASE 2 (B-ligera): actualizar el extremo tras colocar (igual que el
          // loop principal), para que la cadena se arme correctamente.
          if (bb === 0) {
            partida.FichaIzquierda = ficha2;
            partida.FichaDerecha = ficha2;
          } else if (ficha2.Rama === 'izquierda') {
            partida.FichaIzquierda = ficha2;
          } else if (ficha2.Rama === 'derecha') {
            partida.FichaDerecha = ficha2;
          } else {
            if (entry2.side === 'left') partida.FichaIzquierda = ficha2;
            else partida.FichaDerecha = ficha2;
          }
        } catch (err) { console.error('[Domino_Socket] RESET FULL: error colocando', entry2, err); }
      }
